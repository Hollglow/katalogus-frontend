...
export const PaginatedStudentTable = (props) => {
  ...
  const classHeader = props.classId ? <Button sx={{textAlign: "center", display: "block", width: "fit-content", margin: "0px auto 10px", fontSize: 30}} component={RouterLink} variant='outlined' size='large' to ={`/classes/${props.classId}`}>
                                        {props.classId}
                                      </Button>
                                      :
                                      null;
  
  const filteredData = props.filter ? Object.fromEntries(Object.entries(props.data).filter(([id, student]) => student.toLowerCase().includes(props.filter.toLowerCase()))) : props.data;
  ...
  return (
    ...
    <TableContainer component={Paper} sx={{margin: props.margin ? '' : 'auto', maxWidth:'50%', minWidth: '30%'}}>
      <Table aria-label="paginated table">
        ...
        <TableBody>
          {(rowsPerPage > 0
            ? Object.entries(filteredData).slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
