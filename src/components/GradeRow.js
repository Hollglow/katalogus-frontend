  ...
  return(
    ...
    {props.edit && <IconButton aria-label="edit" sx={{padding: 0, margin: "0 10px"}} onClick={handleOpenDialog}>
      ...
      <Box sx={{ verticalAlign: "middle"}}>
        <Typography display="inline" sx={{fontSize: 16}}>{props.grade.Jegy} - </Typography>
        <Typography display="inline" sx={{color: "gray", fontSize: 10}}>{props.grade.Datum.toDate().toLocaleDateString("en-US")}</Typography>
      </Box>
    </Stack>
    <Dialog
        open={open}
      ...
      >
        ...
        <DialogContent>
          <DialogContentText id="alert-dialog-description">
            ...
            <Typography>Dátum: {props.grade.Datum.toDate().toLocaleDateString("en-US")}</Typography>
          </DialogContentText>
        </DialogContent>
        <DialogActions>
